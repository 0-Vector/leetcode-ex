 ...
 *
 */
public class L789EscapeTheGhosts {
    public boolean escapeGhosts(int[][] ghosts, int[] target) {
        //计算阻碍者和目的地之间的欧几里得距离
        int [] lens =  new int[ghosts.length];
        for(int i = 0;i<ghosts.length;i++){
            lens[i] = Math.abs(ghosts[i][0]-target[0]) +Math.abs(ghosts[i][1]-target[1]);
        }
        int sourceLen = Math.abs(target[0])+Math.abs(target[1]);
        for(int len:lens){
            if(len <= sourceLen){
                return false;
            }
